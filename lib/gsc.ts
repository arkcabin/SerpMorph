...
export async function getGscUserInfo(tokens: Auth.Credentials) {
...
export async function syncSitePerformance(siteId: string, days: number = 30) {
  ...
  if (!site || !site.gscAccount)
    throw new Error("Site or associated GSC account not found")

  const auth = getGscClient(
    site.gscAccount.accessToken,
    site.gscAccount.refreshToken
  )
  const searchconsole = google.searchconsole({ version: "v1", auth })
...
export async function getVolatilityScore(siteId: string) {
  ...
  const movements = performance
    .slice(0, -1)
    .map((p, i) => Math.abs(p.position - performance[i + 1].position))
...
export async function getSitemapStatus(siteId: string) {
  ...
  const auth = getGscClient(
    account.gscAccount.accessToken,
    account.gscAccount.refreshToken
  )
